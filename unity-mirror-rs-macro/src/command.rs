...
pub(crate) fn command_handler(
    ...
    // 解析属性参数
    let mut requires_authority = match attr.parse_args::<CommandArgs>() {
        Ok(command_args) => {
            // function_full_name = command_args.function_full_name;
            command_args.requires_authority.is_some()
                && bool::from_str(&command_args.requires_authority.unwrap()).unwrap_or(false)
        }
        Err(_) => false,
    };
    ...
    for arg in sign.inputs.iter().filter_map(|input| match input {
        ...
        match utils::type_to_csharp(&arg.ty) {
            None => {
                params.push(
                    quote! {&format!("{}.{}{}",#param_type::get_namespace(),#param_type::get_prefix(), #param_type_name)},
                );
            }
            Some(r#type) => {
    ...
    let params_read_to: Vec<TokenStream> = (0..params.len())
        .map(|_| quote! {reader.read_to()})
        .collect();
    ...
    comb.add_invoke_user_code(quote! {
        fn #invoke_user_code_method_name(
            ...
            obj.as_any_mut().downcast_mut::<Self>().unwrap().#method_ident(#(#params_read_to,)*);
        }
    });
